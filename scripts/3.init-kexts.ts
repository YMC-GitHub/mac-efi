...
const {log} =console
const database:string="database-kexts"
// const database:string="C:\\Users\\Administrator\\tempocat"

log(`[task] copy Kexts from ocat database`)
...
// - [x] run xxx.ts file with repo esno `pnpm add -D esno` `node_modules/.bin/esno scripts/3.init-kexts.ts`
// - [ ] run xxx.ts file with remo esno `npx esno scripts/3.init-kexts.ts`
